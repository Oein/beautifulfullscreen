...
async function getNeteaseLyrics(info: TrackInfo) {
  const searchURL =
  ...
  let itemId = items.findIndex(
    (val: any) => normalize(val.album.name) === expectedAlbumName
  );
  if (itemId === -1)
    itemId = items.findIndex(
      (val: any) => Math.abs(info.duration - val.duration) < 3000
    );
  if (itemId === -1)
    itemId = items.findIndex((val: any) => val.name === cleanTitle);
  if (itemId === -1) throw "Cannot find track";

  return await Spicetify.CosmosAsync.get(
    lyricURL + items[itemId].id,
    null as any,
    requestHeader
  );
}
...
function breakdownLine(text: string) {
...
function getTranslation(list: any) {
...
export { getNeteaseLyrics, getKaraoke, getSynced, getUnsynced, getTranslation };
